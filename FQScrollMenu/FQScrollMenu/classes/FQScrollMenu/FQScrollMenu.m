...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        [_contentScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(weakSelf.scrollHeader ? superView : weakSelf.headScrollView.mas_bottom);
            make.left.mas_equalTo(0);
            make.width.mas_equalTo(superView.frame.size.width);
            make.height.mas_equalTo(superView.frame.size.height);
        }];
...
- (void)setMenuButtons:(NSArray<FQMenuButton *> *)menuButtons
{
    ...
    for (NSInteger i = 0; i < menuButtons.count; i++) {
        ...
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.height.equalTo(button.superview.mas_height);
...
- (void)setupChildViewControllerWithItem:(FQMenuButton *)item
{
    ...
    [viewController.view mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(weakSelf.isScrollHeader ? weakSelf.superViewController.view : weakSelf.mas_bottom);
        make.left.mas_equalTo(left);
        make.width.equalTo(weakSelf.superViewController.view);
        make.bottom.equalTo(weakSelf.superViewController.view).with.offset(-self.bottomHeight);
    }];
